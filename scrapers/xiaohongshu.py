...
class XiaohongshuScraper(BaseScraper):
    ...
    async def scrape(self, url: str) -> dict:
        ...
        try:
            ...
            post_content = ""
            try:
                # First click expand button if present
                await page.evaluate("""
                    () => {
                        // Find expand button in note content area
                        const expandBtns = document.querySelectorAll('.show-more, [class*="expand"], button');
                    ...
                    () => {
                        ...
                        for (const sel of selectors) {
                            ...
                            if (el && (el.textContent || '').trim().length > 5) {
                                ...
                                text = text.replace(/展开/g, '').replace(/收起/g, '');
                                return text.trim();
                            }
                        }
                        return '';
        ...
        items = await page.evaluate("""
            () => {
                ...
                return Array.from(items).map(item => {
                    ...
                    // Handle image-only comments: if no text but has image, use placeholder
                    ...
                    // Also check the full item for images if content is still empty
                    if (!content) {
                        const images = item.querySelectorAll('img');
                        // Filter out avatar images (typically small or in specific containers)
                        const nonAvatar = Array.from(images).filter(img => {
                            const parent = img.closest('.avatar, [class*="avatar"], [class*="head"]');
